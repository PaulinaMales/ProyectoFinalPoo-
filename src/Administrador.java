...
public class Administrador extends JFrame{
    ...
    public Administrador() {
        ...
        //FIN DE ACCION DE CADA BOTON
    }
    //FIN DE CONSTRUCTOR

    //FUNCION PARA AGRUPAR LOS BOTONES
    public void RadioBTN(){
        ButtonGroup botones = new ButtonGroup();
        botones.add(cajeroRadioButton);
        botones.add(bodegueroRadioButton);
    }
    //FIN DE AGRUPAR BOTONES
    ...
    public void actualizar(){
        ...
        try{
            Connection conn= DriverManager.getConnection(DB_URL,USERNAME,PASSWORD);
            Statement stmt= conn.createStatement();
            String sql="update cajeros set email=?, nombre=?,direccion=?,celular=?, password=? where cedula=?";
            PreparedStatement pst=conn.prepareStatement(sql);
    ...
    public void actualizar1(){
        ...
        try{
            Connection conn= DriverManager.getConnection(DB_URL,USERNAME,PASSWORD);
            Statement stmt= conn.createStatement();
            String sql="update bodegueros set email=?, nombre=?,direccion=?,celular=?, password=? where cedula=?";
            PreparedStatement pst=conn.prepareStatement(sql);
            pst.setString(1,email);
            pst.setString(2,nombre);
            pst.setString(3,direccion);
